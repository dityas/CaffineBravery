...
public class ModelsParser extends SpuddXBaseVisitor<Model> {
	...
	public Model visitPOMDPDef(SpuddXParser.POMDPDefContext ctx) {
		...
		ctx.pomdp_def().dynamics()
            .action_model()
            .stream()
            .forEach(a -> 
                    dynamics.put(a.action_name().getText(), 
                        this.visit(a.all_def())));
		...
		float discount = 
            Float.valueOf(ctx.pomdp_def().discount().FLOAT_NUM().getText());
	...
	public Model visitIPOMDPDef(SpuddXParser.IPOMDPDefContext ctx) {
		...
		var S = ctx.ipomdp_def()
            .states_list()
            .var_name().stream()
            .map(s -> s.getText())
            .collect(Collectors.toList());

		var O = ctx.ipomdp_def()
            .obs_list()
            .var_name().stream()
            .map(o -> o.getText())
            .collect(Collectors.toList());
		...
		var wrongFrames = _frames.stream()
            .filter(f -> !(f._1() instanceof PBVISolvablePOMDPBasedModel))
            .findFirst();
		...
		ctx.ipomdp_def().dynamics().action_model()
            .stream().forEach(a -> 
                    dynamics.put(
                        a.action_name().getText(), this.visit(a.all_def())));

		ctx.ipomdp_def().reward().action_reward().stream()
            .forEach(ar -> {
		...
		var ipomdp = 
            new IPOMDP(S, O, A, Aj, Mj, EC, Thetaj,
                    _frames, dynamics, R, discount, H, 
