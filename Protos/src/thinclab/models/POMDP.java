...
public class POMDP extends PBVISolvablePOMDPBasedModel {
    ...
    // ------------------------------------------------------------------------
    // Implement confirmation biases

    public List<DD> evidenceFactors(DD o) {

        var vars = new ArrayList<>(o.getVars());
        LOGGER.debug("Factorizing %s with %s", o, vars);
        return vars.size() > 1 ? 
    ...
    public List<Tuple3<Integer, DD, Float>> computeNextBaParallel(DD b,
        ...
        var res = IntStream.range(0, oAll.size()).parallel().boxed()
            .map(o -> Tuple.of(
                        DDOP.restrict(likelihoods, i_Om_p, oAll.get(o)).getVal(), 
                        o))
            .filter(o -> o._0() > 1e-6f)
            .map(o ->
                    {
    ...
    public AlphaVector backup(DD b,
        ...
        for (int a = 0; a < A().size(); a++) {
            ...
            for (int o = 0; o < nextBa.size(); o++) {
                ...
                var bestAlphaVal = DDOP.dotProduct(
                        Vn.get(bestAlphaIndex).getVector(), 
                        b_n, Vn.stateIndices);
