...
public class IPOMDP extends PBVISolvablePOMDPBasedModel {
    ...
    public List<Tuple3<
        Integer, PBVISolvablePOMDPBasedModel, List<ReachabilityNode>>> framesj;
            ...
            .forEach(e ->
                    {
                        ...
                        if (ec == null) {
                            LOGGER.error("Panic! Could not find equivalence" +
                                    " class for belief %s", e.getValue());
                            LOGGER.debug("actId is %s, alphaId %s and " +
                                    "frameId %s, act %s", 
                                    actId, alphaId, frameId, 
                                    frame.m.A().get(actId));
                            LOGGER.debug("EC map is %s", ECMap);
                            LOGGER.debug("Belief is %s", bel);
    ...
    public IPOMDP(List<String> S, List<String> O, String A, 
        ...
        if (incorrectFrame.isPresent()) {
            ...
            LOGGER.error("All frames should have the same set of observation" +
                    " variables. %s seems to be different.",
                    incorrectFrame.get());
            System.exit(-1);
        }
        ...
        var OjRestrictedAi = IntStream
            .range(0, A().size())
            .mapToObj(i -> this.framesj.stream()
                    .map(f -> this.prepareOj_pGivenAjAiS_p(i + 1, f._1().O()))
                    .collect(Collectors.toList()))
            .collect(Collectors.toList());
        ...
        this.ecThetas = this.framesj.stream()
            .map(f -> new MjThetaSpace(f._2().stream()
                        .flatMap(n -> n.beliefs.stream())
                        .collect(Collectors.toList()), f._0(), f._1()))
            .collect(Collectors.toList());
    ...
    public IPOMDP(List<String> S, List<String> O, String A, 
        ...
        this.framesj = frames_j.stream()
            ...
            .map(t -> Tuple.of(
                        t._0(), t._1(), 
                        Global.modelVars.get(
                            Global.varNames.get(i_Mj - 1)).keySet()
                        .stream()
                        .filter(k -> k.frame == t._0())
                        .map(k -> k._1())
                        .collect(Collectors.toList())))
        ...
        // verify and prepare Oj
        var incorrectFrame = this.framesj.stream()
            .filter(f -> !f._1().Om().equals(this.framesj.get(0)._1().Om()))
            .findAny();

        if (incorrectFrame.isPresent()) {
            LOGGER.error("All frame should have the same set of observation" +
                    " variables. %s seems to be different.",
                    incorrectFrame.get());
            System.exit(-1);
        }
    ...
    public List<DD> prepareOj_pGivenAjAiS_p(int ai, List<List<DD>> _Oj) {
        ...
        var Oj_ = _Oj.stream()
            .map(oja -> oja.stream()
                    .map(oj -> DDOP.restrict(oj, List.of(i_A), List.of(ai)))
                    .collect(Collectors.toList()))
            .collect(Collectors.toList());

        var Oj_pGivenAjAiS_p = IntStream.range(0, Omj.size())
            .mapToObj(i -> 
                    DDOP.reorder(DDnode.getDD(i_Aj, 
                            IntStream.range(0, Aj.size())
                            .mapToObj(j -> Oj_.get(j).get(i))
                            .toArray(DD[]::new))))
            .collect(Collectors.toList());
    ...
    public DD getECDDFromMjDD(DD mjDD) {
        ...
        // accumulate
        var _ecDDMap = Global.valNames.get(i_EC - 1).stream()
            .collect(Collectors.toMap(v -> v, v -> DD.zero));

        IntStream.range(0, Global.valNames.get(i_Mj - 1).size())
            .forEach(i ->
                    {
        ...
        var ecDD = DDnode.getDD(i_EC,
                Global.valNames.get(i_EC - 1).stream()
                .map(v -> _ecDDMap.get(v))
                .toArray(DD[]::new));
    ...
    public void updateIS() {
        ...
        R = jointR.stream()
            .map(jr -> DDOP.addMultVarElim(
                        List.of(PAjGivenEC, jr), List.of(i_Aj)))
            .collect(Collectors.toList());
    }
    ...
    public void createIS() {
        ...
        var ecList = this.ecThetas.stream()
            .flatMap(m -> m.allModels().stream())
            .collect(Collectors.toList());

        IntStream.range(0, ecList.size())
            .forEach(i ->
                    {
        ...
        LOGGER.debug("Eq class space for %s has %s models", 
                getName(), Global.valNames.get(i_EC - 1).size());
        LOGGER.debug("IPOMDP %s has %s interactive states in total", 
                getName(), i_S().stream()
    ...
    public void createPAjEC() {
        ...
        Predicate<DD> isInvalidProbability = dd ->
            DDOP.maxAll(
                    DDOP.abs(
                        DDOP.sub(
        ...
        var optAGivenEC = ecThetas.stream()
            .flatMap(mj -> mj.allModels().stream())
            .collect(Collectors.toMap(mj -> ECMap.get(mj), 
                        mj -> DDnode.getDDForChild(i_Aj, mj._1().actId)));
        ...
        // Sanity check
        IntStream.range(0, Global.valNames.get(i_EC - 1).size())
            .forEach(i ->
                    {
                        var dd = DDOP.restrict(PAjGivenEC, 
                                List.of(i_EC), List.of(i + 1));
                        if (isInvalidProbability.test(dd)) {
                            LOGGER.debug("Panic! Sanity check for P(Aj|EC)" +
                                    " failed, P(Aj|EC) = %s", PAjGivenEC);
                            System.exit(-1);
                        }
                    });
    ...
    public void createPThetajEC() {
        ...
        var mjToThetaj = ecThetas.stream()
            .flatMap(mj -> mj.allModels().stream())
            .collect(Collectors.toMap(
                        mj -> ECMap.get(mj), 
                        mj -> DDnode.getDDForChild(i_Thetaj, mj.frame)));
        ...
        IntStream.range(0, Global.valNames.get(i_EC - 1).size())
            .forEach(i ->
                    {
                        var dd = DDOP.restrict(PThetajGivenEC,
                                List.of(i_EC), List.of(i + 1));
                        if (isInvalidProbability.test(dd)) {
                            LOGGER.debug(
                                    "Panic! Sanity check for P(Thetaj|EC)" +
                                    " failed, P(Thetaj|EC) = %s",
                                    PThetajGivenEC);
                            System.exit(-1);
                        }
                    });
    ...
    public void createPEC_pECAjOj_p() {
        ...
        var triples = ecThetas.stream()
            .flatMap(f -> f.getTriples().stream())
            .map(f -> Tuple.of(ECMap.get(f._0()), f._1(), ECMap.get(f._2())))
            .map(t ->
                    {
                        // convert everything into list
                        List<Integer> valList = 
                            new ArrayList<>(t._1().size() + 2);

                        valList.add(Global.valNames.get(i_EC - 1)
                                .indexOf(t._0()) + 1);
                        valList.addAll(t._1());
                        valList.add(Global.valNames.get(i_EC_p - 1)
                                .indexOf(t._2()) + 1);
                        return valList;
                    })
        .collect(Collectors.toList());
        ...
        PEC_pGivenECAjOj_p = triples.stream()
            .map(t -> DDOP.reorder(DDnode.getDD(varList, t, 1.0f)))
            .reduce(DDleaf.getDD(0.0f), (d1, d2) -> DDOP.add(d1, d2));

        var ddSum = DDOP.addMultVarElim(
                List.of(PEC_pGivenECAjOj_p), List.of(i_EC_p));

        if (DDOP.maxAll(DDOP.abs(DDOP.sub(ddSum, DD.one))) > 1e-5f) {
            LOGGER.debug("Models are %s", Global.modelVars);
            LOGGER.error("# (EC') P(EC'|EC, Aj, Oj') is %s != 1", ddSum);
            System.exit(-1);
        }

        LOGGER.info("Sanity check passed for P(EC'|EC, Aj, Oj')");

        Taus = IntStream.range(0, A().size())
            .mapToObj(i ->
                    {
        ...
        LOGGER.debug("Building P(EC'|EC, Aj, Oj') for %s took %s msecs",
                getName(), ((now - then) / 1000000.0));
    }
    ...
    public DD beliefUpdate(DD b, int a, List<Integer> o) {
        ...
        if (getBiased())
            return beliefUpdateBiased(b, a, o);

        else
            return beliefUpdateRational(b, a, o);
    ...
    public DD beliefUpdateRational(DD b, int a, List<Integer> o) {

        var Ofao = DDOP.restrict(O().get(a), i_Om_p, o);

        var factors = 
            new ArrayList<DD>(S().size() + S().size() + Omj.size() + 3);
        ...
        var b_p = DDOP.primeVars(
                DDOP.addMultVarElim(factors, vars), -(Global.NUM_VARS / 2));
    ...
    public DD beliefUpdateRational(DD b, String a, List<String> o) {
        ...
        for (int i = 0; i < i_Om_p.size(); i++) {
            ...
            obs.add(Collections.binarySearch(
                        Global.valNames.get(i_Om.get(i) - 1), o.get(i)) + 1);
        }

        return this.beliefUpdateRational(b, actIndex, obs);
    }
    ...
    // -----------------------------------------------------------------------
    // Confirmation bias implementations for IPOMDPs
    ...
    public DD beliefUpdateBiased(DD b, String a, List<String> o) {
        ...
        return this.beliefUpdateBiased(b, actIndex, obs);
    }
    ...
    public List<Tuple3<Integer, DD, Float>> computeNextBaParallel(DD b, 
            DD likelihoods, int a, ReachabilityGraph g) {

        var res = IntStream.range(0, oAll.size()).parallel().boxed()
            .map(o -> Tuple.of(
                        DDOP.restrict(
                            likelihoods, i_Om_p, oAll.get(o)).getVal(), 
                        o))
            .filter(o -> o._0() > 1e-6f)
            .map(o ->
                    {
    ...
    public Tuple3<Integer, DD, Float> getBestAlpha(Tuple3<Integer, DD, Float> nextBa,
        ...
        return Tuple.of(obsIndex, alphas.get(bestAlpha._1()), 
                (prob * bestAlpha._0()));
    }
    ...
    public AlphaVector backup(DD b, AlphaVectorPolicy Vn, ReachabilityGraph g) {
        ...
        var newVec = 
            DDOP.add(R().get(bestA), DDOP.mult(DDleaf.getDD(discount), vec));
