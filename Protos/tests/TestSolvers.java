...
class TestSolvers {
	...
	void testL2IPOMDPSolution() throws Exception {
		...
		var b_i = domainRunner.getDD("l2Belief");
        LOGGER.debug("Bi is %s", J.getECDDFromMjDD(b_i));

		var policy = new SymbolicPerseusSolver<>(J)
            .solve(List.of(J.getECDDFromMjDD(b_i)), 100, J.H);
		int bestAct = policy.getBestActionIndex(b_i);

		LOGGER.info("Suggested optimal action for tiger problem is %s which " +
                "resolves to %s", bestAct, J.A().get(bestAct));
